...
/* ── Publisher Teams Tab ─────────────────────────────────────── */
function PublisherTeamsTab({ token }) {
  const [teams, setTeams] = useState([]);
  const [loading, setLoading] = useState(false);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="table-container">
        {loading ? (
          ...
          <table className="admin-table">
            <thead>
              <tr><th>ID</th><th>Team Name</th><th>Host Subdomain</th><th>Commission Rate</th><th style={{ textAlign: 'right' }}>Actions</th></tr>
            </thead>
            <tbody>
              {teams.map((t) => (
                <tr key={t.team_id}>
                  ...
                  <td>
                    {t.team_host ? (
                      <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'var(--primary-brand)', fontSize: 13, fontWeight: 600 }}>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                      <button onClick={() => handleOpenEdit(t)} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--text-color)', padding: 0 }}>
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(t.team_id)} style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#EF4444', padding: 0 }}>
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>
...
const TABS = [
  ...
  { id: 'teams', label: 'Publisher Teams', icon: Users },
];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }} className="anima-fade-in">
      ...
      <div>
        ...
        {activeTab === 'teams' && <PublisherTeamsTab token={token} />}
      </div>
    </div>
  );
}
